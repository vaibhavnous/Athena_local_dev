...
Source: insurance.dbo.claim_information
Expected runtime: Spark / Databricks with Delta support
Target table: main.bronze.bronze_claim_information
...
TARGET_TABLE = "bronze.bronze_claim_information"
TEMP_VIEW = "bronze_src_claim_information"
CAST_RULES = {'claimid': 'bigint', 'lossdate': 'timestamp', 'inserteddate': 'timestamp', 'updatenum': 'int', 'claimclosedate': 'timestamp', 'courtid': 'bigint', 'claimnaturesoflossid': 'bigint'}
...
df = (
    ...
    .option("dbtable", "dbo.claim_information")
    .option("driver", "com.microsoft.sqlserver.jdbc.SQLServerDriver")
    .load()
)

if not df.schema or not df.schema.fields:
    raise ValueError("Source read returned an empty schema for insurance.dbo.claim_information.")
...
df = (
    ...
    .withColumn("source_table", lit("claim_information"))
)
