...
Source: insurance.dbo.indemnity_outstanding_estimates
Expected runtime: Spark / Databricks with Delta support
Target table: main.bronze.bronze_indemnity_outstanding_estimates
...
TARGET_TABLE = "bronze.bronze_indemnity_outstanding_estimates"
TEMP_VIEW = "bronze_src_indemnity_outstanding_estimates"
CAST_RULES = {'claimid': 'bigint', 'updatenum': 'int', 'grossestimate': 'decimal(18,2)', 'inserteddate': 'timestamp'}
...
df = (
    ...
    .option("dbtable", "dbo.indemnity_outstanding_estimates")
    .option("driver", "com.microsoft.sqlserver.jdbc.SQLServerDriver")
    .load()
)

if not df.schema or not df.schema.fields:
    raise ValueError("Source read returned an empty schema for insurance.dbo.indemnity_outstanding_estimates.")
...
df = (
    ...
    .withColumn("source_table", lit("indemnity_outstanding_estimates"))
)
