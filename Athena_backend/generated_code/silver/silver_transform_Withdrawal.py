...
# ============================================================
# Silver Transformation: Vendor1/Withdrawal
# ============================================================

SOURCE_TABLE = "bronze.vendor1_withdrawal_raw"
TARGET_TABLE = "silver.vendor1_withdrawal_clean"
...
# ============================================================
try:
    _last_watermark = spark.sql("SELECT MAX(_silver_processed_at) FROM silver.vendor1_withdrawal_clean").first()[0]
    if _last_watermark:
...
df = source_df.select(
    ...
    F.col("`RemoteMessage_sourceDetails_count`").cast("string").alias("source_details_count"),
...
spark.sql("""CREATE TABLE IF NOT EXISTS silver.vendor1_withdrawal_clean
    USING DELTA
    AS SELECT * FROM _silver_updates WHERE 1=0
""")

spark.sql("""MERGE INTO silver.vendor1_withdrawal_clean AS target
...
row_count = spark.table("silver.vendor1_withdrawal_clean").count()
print(f"Silver transformation completed: silver.vendor1_withdrawal_clean")
