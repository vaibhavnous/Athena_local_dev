  ...
  return <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose} className="fixed inset-0 z-[100] flex justify-end bg-black/60 backdrop-blur-sm">
    <motion.aside initial={{x:440}} animate={{x:0}} exit={{x:440}} onClick={e => e.stopPropagation()} className="flex h-full w-full max-w-md flex-col border-l border-bg-border bg-bg-card shadow-2xl">
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-5">
        ...
        <Field label="Target"><select className="input-field" value={form.target} onChange={e => {const target=e.target.value;setForm(current=>({...current,target,executionEngine:String(target).toLowerCase()==='snowflake'&&current.connectionType==='database'?(current.executionEngine||'native'):'native',dbtDeploymentMode:'generate_only',forceDbtDeploy:false}))}}><option>Databricks</option><option>Snowflake</option><option disabled>Fabric</option></select></Field>
        ...
        {connectionsLoading ? <p className="text-xs text-text-tertiary">Loading configurations...</p> : <>
          <Field label="Source Type"><select className="input-field" value={form.connectionType} onChange={e => {const connectionType=e.target.value;setForm(current=>({...current,connectionType,connectionName:'',dbType:'',databaseName:'',integrationType:'',dataLakeName:'',executionEngine:connectionType==='database'&&String(current.target).toLowerCase()==='snowflake'?(current.executionEngine||'native'):'native',dbtDeploymentMode:'generate_only',forceDbtDeploy:false}))}}><option value="">Select source type...</option><option value="database">Database</option><option value="data_lake">Data Lake</option></select></Field>
          {snowflakeDatabaseTarget && <Field label="Snowflake Engine"><select className="input-field" value={form.executionEngine||'native'} onChange={e=>setForm(current=>({...current,executionEngine:e.target.value,dbtDeploymentMode:'generate_only',forceDbtDeploy:false}))}><option value="native">Native</option><option value="dbt">dbt</option></select><p className="mt-1 text-xs text-text-tertiary">Generate dbt project artifacts for review. Execution and deployment are not enabled yet.</p></Field>}
