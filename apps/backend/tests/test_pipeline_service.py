...
def test_stage_local_failure_stops_database_continuation(monkeypatch):
    from services import pipeline_runtime
    ...
    def fake_runner(stage_key):
        def _run(state):
            ...
            return {"metadata_status": "FAILED", "metadata_error": "catalog unavailable"}
    ...
    result = pipeline_runtime.continue_database_pipeline(
        ...
        state={"run_id": "run-metadata-failed"},
    )

    assert visited == ["discovery"]
    assert result["status"] == "FAILED"
    assert result["failed_background_stage"] == "discovery"
    assert result["error"] == "catalog unavailable"
    assert result.get("last_completed_stage_key") is None
...
def test_generation_skipped_is_retryable_failure():
    ...
    result = pipeline_runtime._normalize_stage_failure(
        "silver",
        {"run_id": "run-silver", "silver_generation_status": "SKIPPED"},
    )

    assert result["status"] == "FAILED"
    assert result["failed_background_stage"] == "silver"
...
def test_seed_payload_from_checkpoint_heals_legacy_choices():
    checkpoint = {
        ...
        "brd_text": "legacy brd",
        "source": "unknown",
        "target_warehouse": "unknown",
        "sftp_entity": "bad",
        "database_name": 123,
        "compliance_countries": "US",
    }

    payload = pipeline_service.seed_payload_from_checkpoint(checkpoint)

    assert payload.source == "database"
    assert payload.target_warehouse == "databricks"
    assert payload.sftp_entity == "transactions"
    assert payload.database_name == "123"
    assert payload.compliance_countries == ["US"]
...
def test_load_checkpoint_state_returns_failed_state_for_malformed_json(monkeypatch):
    from services import pipeline_runtime
    ...
    checkpoint = pipeline_runtime.load_checkpoint_state("run-bad-json")

    assert checkpoint["status"] == "FAILED"
    assert checkpoint["error_type"] == "MalformedCheckpoint"
    assert checkpoint["checkpoint_malformed"] is True
...
def test_databricks_gate4_always_pauses_for_merge_key_review_when_execution_disabled(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (_ for _ in ()).throw(AssertionError("Silver must wait for merge-key approval")),
    )
