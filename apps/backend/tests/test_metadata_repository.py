...
def test_full_enqueue_persists_exact_runtime_context_and_rejects_incremental() -> None:
    ...
    repository.runtime_object.update({
        "load_type": "FULL",
    ...
    file_spec = json.loads(repository.runtime_object["execution_spec_json"])
    file_spec["source_file"] = {
        "path": "INSURANCE_SFTP/insurance/claims.csv",
        "format": "CSV",
    }
    repository.runtime_object.update({
        "ingestion_type": "FILE",
        "source_path": "INSURANCE_SFTP/insurance/claims.csv",
        "payload_format": "CSV",
        "execution_spec_json": json.dumps(file_spec),
    })
    file_queue = repository.enqueue_work(
        ingestion_object_id=101,
        trigger_type="MANUAL",
        work_scope={"design_run_id": "file-design-1"},
        requested_by="tester",
        logical_work_id="logical-file-1",
    )
    assert file_queue["logical_work_id"] == "logical-file-1"
