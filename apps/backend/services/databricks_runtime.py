...
def _persist_databricks_submission(
    state: Dict[str, Any],
    *,
    layer: str,
    run_id: str,
    ...
    submitted = {
        **state,
        ...
        f"databricks_{layer}_execution_status": "RUNNING",
        f"databricks_{layer}_external_run_id": databricks_run_id,
        f"databricks_{layer}_submitted_at": datetime.now(timezone.utc).isoformat(),
        "external_execution": {
            **(state.get("external_execution") if isinstance(state.get("external_execution"), dict) else {}),
            "platform": "databricks",
            "layer": layer,
            "stage_key": stage_key,
            "status": "RUNNING",
        ...
        "resume_message": f"Databricks {layer.capitalize()} run submitted: {databricks_run_id}.",
    }
    try:
        from services.pipeline_runtime import save_checkpoint_state

        save_checkpoint_state(str(run_id), submitted)
    except Exception as exc:
        logger.warning(
            "Failed to persist Databricks submitted run id: %s",
            exc,
            extra={"run_id": run_id, "node": stage_key, "stage": stage_key},
        )
    return submitted
...
def _execute_databricks_stage_batch(
    ...
    *,
    ...
    databricks_run_id = int(run_payload.get("run_id"))
    state = _persist_databricks_submission(
        state,
        layer=layer,
        run_id=run_id,
        notebook_path=notebook_path,
        databricks_run_id=databricks_run_id,
        script_name=f"{layer}_batch",
    )
    run_state = _wait_for_run(databricks_run_id)
