...
def _mapping_rows(
    ...
    for stage, source_layer, target_layer in FILE_STAGES:
        ...
        for ordinal, column in enumerate(columns, start=1):
            raw_source_name = str(column.get("column_name") or column.get("source_field_path") or "").strip()
            if not raw_source_name:
                raise ValueError("Inferred file columns require a non-empty name.")
            target_name = normalize_bronze_column_name(raw_source_name)
            # ponytail: only the file boundary uses vendor headers; every downstream
            # layer reads the canonical alias materialized by the preceding layer.
            source_name = raw_source_name if stage == "SOURCE_TO_BRONZE" else target_name
