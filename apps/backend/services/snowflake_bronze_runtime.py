    ...
    *,
    ...
    try:
        ...
        if load_source and source_mode == "adls":
            ...
            if adls_flow:
                ensure_adls_stage(snowflake_conn, adls_flow=True)
            else:
                ensure_adls_stage(snowflake_conn)
        for index, script in enumerate(scripts, start=1):
            if adls_flow and script.get("metadata_runtime_context"):
            ...
            if load_source:
                ...
                if source_mode == "adls":
                    load_result = (
                        load_adls_table_to_snowflake(script, snowflake_conn, adls_flow=True)
                        if adls_flow
                        else load_adls_table_to_snowflake(script, snowflake_conn)
                    )
