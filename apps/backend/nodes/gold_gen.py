...
def _snowflake_dbt_codegen_enabled(state: Stage01State) -> bool:
    return _target_warehouse(state) == "snowflake" and str(state.get("execution_engine") or "").strip().lower() == "dbt"
...
def gold_code_generation_node(state: Stage01State) -> Stage01State:
    ...
    if dbt_codegen:
        ...
        results = [
            item for item in dbt_state.get("gold_generation_results") or results
            if isinstance(item, dict)
        ]
