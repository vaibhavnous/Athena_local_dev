...
def _adls_demo_fallback_enabled() -> bool:
    return _env_flag("ATHENA_ADLS_DEMO_FALLBACK_ENABLED", "ATHENA_DEMO_MODE")
    ...
    try:
        if source == "sftp":
            ...
            try:
                ...
                if not _adls_demo_fallback_enabled():
                    raise RuntimeError(f"ADLS live ingestion failed: {exc}") from exc
                logger.warning(
