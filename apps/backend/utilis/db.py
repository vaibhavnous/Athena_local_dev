...
def _env_int_any(names: tuple[str, ...], default: int, *, min_value: int | None = None, max_value: int | None = None) -> int:
    for name in names:
...
def _sql_identifier(value: str | None, default: str, *, env_name: str) -> str:
    candidate = str(value or default).strip()
    if re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", candidate):
        return candidate
    logger.warning("Invalid SQL identifier in %s=%r; using %s", env_name, value, default)
    return default
...
config = {
    "azure_sql": {
        ...
        "port": _env_int("AZURE_SQL_PORT", 1433, min_value=1, max_value=65535),
        ...
        "connection_timeout": _env_int_any(
            ("AZURE_SQL_CONNECTION_TIMEOUT", "ATHENA_SQL_CONNECTION_TIMEOUT_SECONDS"),
            5,
            min_value=1,
            max_value=300,
        ),
        ...
        "pipeline_schema": _sql_identifier(os.getenv("AZURE_SQL_PIPELINE_SCHEMA"), "metadata", env_name="AZURE_SQL_PIPELINE_SCHEMA"),  # ✅ YOUR SCHEMA
        "schema_name": _sql_identifier(os.getenv("AZURE_SQL_PIPELINE_SCHEMA"), "metadata", env_name="AZURE_SQL_PIPELINE_SCHEMA"),  # backward-compat alias

        # 🔥 CLIENT DB (FAKE CLIENT)
        "source_database": os.getenv("AZURE_SQL_SOURCE_DATABASE", "insurance"),
        "source_schema": _sql_identifier(os.getenv("AZURE_SQL_SOURCE_SCHEMA"), "dbo", env_name="AZURE_SQL_SOURCE_SCHEMA"),  # ✅ CLIENT SCHEMA
...
SQL_CONNECT_RETRIES = _env_int("ATHENA_SQL_CONNECT_RETRIES", 1, min_value=1, max_value=10)
SQL_CONNECT_RETRY_DELAY_SECONDS = _env_float("ATHENA_SQL_CONNECT_RETRY_DELAY_SECONDS", 1.0, min_value=0.0, max_value=60.0)
SQL_TCP_PROBE_TIMEOUT_SECONDS = _env_float("ATHENA_SQL_TCP_PROBE_TIMEOUT_SECONDS", 2.0, min_value=0.1, max_value=30.0)
...
SQL_ENDPOINT_NEGATIVE_CACHE_SECONDS = _env_float("ATHENA_SQL_ENDPOINT_NEGATIVE_CACHE_SECONDS", 60.0, min_value=0.0, max_value=3600.0)
