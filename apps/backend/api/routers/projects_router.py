...
def _payload(request: ProjectRequest, owner_email: str) -> dict[str, Any]:
    ...
    data["dbt_deployment_mode"] = str(data.get("dbt_deployment_mode") or "generate_only").strip().lower()
    ...
    if data["target"] != "Snowflake" or data["connection_type"] != "database" or data["execution_engine"] != "dbt":
