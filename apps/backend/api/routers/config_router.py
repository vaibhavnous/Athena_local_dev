...
from api.models import ConfigurationPayload, SettingsPayload
...
def _test_database_configuration(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if db_type == "azure_sql" and _configured_source_matches(host, port, database_name):
        ...
        return {"success": True, "tested": True, "message": "Configured Azure SQL endpoint is reachable."}
...
@router.put("/settings")
def save_settings(data: SettingsPayload) -> Dict[str, Any]:
...
def create_configuration(data: ConfigurationPayload) -> Dict[str, Any]:
    payload = data.to_payload()
    return {**_redact_sensitive(payload), "id": _configuration_id(payload)}


@router.post("/configurations/test")
def test_configuration(data: ConfigurationPayload) -> Dict[str, Any]:
    payload = data.to_payload()
    source_type = _field(payload, "source_type", "sourceType", default="database").lower()
    if source_type == "data_lake":
        return _test_data_lake_configuration(payload)
    return _test_database_configuration(payload)


@router.put("/configurations/{config_id}")
def update_configuration(config_id: str, data: ConfigurationPayload) -> Dict[str, Any]:
    return {**_redact_sensitive(data.to_payload()), "id": config_id}
