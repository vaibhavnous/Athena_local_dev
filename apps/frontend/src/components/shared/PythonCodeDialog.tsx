...
import { normalizeRunScripts } from '../../utils/codeReviewArtifacts'
...
export default function PythonCodeDialog({ isOpen, onClose, stageName, runId, title }) {
  ...
  useEffect(() => {
    ...
    getRunScripts(runId)
      .then(payload => {
        if (!cancelled) {
          setFiles(normalizeRunScripts(payload, layer).map(file => ({
  ...
  return <AnimatePresence>{isOpen && <>
    ...
    <motion.section initial={{opacity:0,scale:.97,y:12}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0,scale:.97,y:12}} className="fixed inset-x-3 bottom-[5%] top-[5%] z-50 mx-auto flex max-w-5xl flex-col overflow-hidden rounded-2xl border border-bg-border bg-bg-card shadow-2xl sm:inset-x-6">
      ...
      {files.length>0&&<nav className="flex overflow-x-auto border-b border-bg-border bg-bg-base">{files.map((file,index)=><button key={`${file.filename}:${index}`} onClick={()=>setActiveTab(index)} className={`flex items-center gap-1.5 border-r border-bg-border px-4 py-2.5 font-mono text-xs ${index===activeTab?'border-t-2 border-t-accent-blue bg-bg-card text-white':'border-t-2 border-t-transparent text-text-muted'}`}><FileCode2 size={12}/>{file.filename}</button>)}</nav>}
      <div className="min-h-0 flex-1 overflow-auto bg-[#282c34]">
        ...
        {!loading&&activeFile&&<div className="min-h-full min-w-max py-5 font-mono text-[0.8125rem] leading-[1.6]">{String(activeFile.code || '').split('\n').map((line,index)=><div key={index} className="grid grid-cols-[52px_1fr] px-3 hover:bg-white/[0.025]"><span className="select-none pr-4 text-right text-[#667287]">{index+1}</span><span className="whitespace-pre pr-6 text-[#abb2bf]">{line || ' '}</span></div>)}</div>}
      </div>
      {activeFile&&<footer className="flex items-center justify-between border-t border-bg-border bg-bg-base px-5 py-2 text-[10px] text-text-muted"><div className="flex items-center gap-3 font-mono"><span>{activeFile.filename}</span><span className="rounded border border-[#274c86] bg-[#102348] px-1.5 py-0.5 text-[#5797ff]">{String(activeFile.filename).split('.').pop() || 'code'}</span></div><span>{String(activeFile.code||'').split('\n').length} lines</span></footer>}
    </motion.section>
  </>}</AnimatePresence>
}
