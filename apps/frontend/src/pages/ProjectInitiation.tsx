  ...
  return <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose} className="fixed inset-0 z-[100] flex justify-end bg-black/60 backdrop-blur-sm">
    <motion.aside initial={{x:440}} animate={{x:0}} exit={{x:440}} onClick={e => e.stopPropagation()} className="flex h-full w-full max-w-md flex-col border-l border-bg-border bg-bg-card shadow-2xl">
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-5">
        ...
        <Field label="Target"><select className="input-field" value={form.target} onChange={e => {const target=e.target.value;setForm(current=>{const snowflakeDatabase=String(target).toLowerCase()==='snowflake'&&current.connectionType==='database';return {...current,target,executionEngine:snowflakeDatabase?current.executionEngine:'native',dbtDeploymentMode:snowflakeDatabase&&current.executionEngine==='dbt'?'generate_and_deploy':'generate_only',forceDbtDeploy:false}})}}><option>Databricks</option><option>Snowflake</option><option disabled>Fabric</option></select></Field>
