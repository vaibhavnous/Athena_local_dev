...
export function supportsSnowflakeEngine(form) {
  const target = String(form.target || '').trim().toLowerCase()
  const connectionType = String(form.connectionType || '').trim().toLowerCase()
  const integrationType = String(form.integrationType || '').trim().toLowerCase()
  return target === 'snowflake' && (
    connectionType === 'database' ||
    (connectionType === 'data_lake' && integrationType === 'adls')
  )
}
...
export function ProjectForm({ initial, connections, connectionsLoading, busy, onClose, onSave }) {
  ...
  const snowflakeSupportedTarget = supportsSnowflakeEngine(form)
    ...
    <motion.aside initial={{x:440}} animate={{x:0}} exit={{x:440}} onClick={e => e.stopPropagation()} className="flex h-full w-full max-w-md flex-col border-l border-bg-border bg-bg-card shadow-2xl">
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-5">
        ...
        {connectionsLoading ? <p className="text-xs text-text-tertiary">Loading configurations...</p> : <>
          ...
          <Field label="Target"><select className="input-field" value={form.target} onChange={e => {const target=e.target.value;setForm(current=>{const snowflakeSupported=supportsSnowflakeEngine({...current,target});return {...current,target,executionEngine:snowflakeSupported?current.executionEngine:'native',dbtDeploymentMode:snowflakeSupported&&current.executionEngine==='dbt'?'generate_and_deploy':'generate_only',forceDbtDeploy:false}})}}><option>Databricks</option><option>Snowflake</option><option disabled>Fabric</option></select></Field>
