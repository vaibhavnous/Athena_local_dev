...
const normalizeChoice = (value, fallback = '') => String(value || fallback).trim().toLowerCase()
...
export function ProjectForm({ initial, connections, connectionsLoading, busy, onClose, onSave }) {
  ...
  const targetValue = normalizeChoice(form.target)
  const connectionTypeValue = normalizeChoice(form.connectionType)
  const executionEngineValue = normalizeChoice(form.executionEngine, 'native')
  const dbtDeploymentModeValue = normalizeChoice(form.dbtDeploymentMode, 'generate_only')
  const snowflakeDatabaseTarget = targetValue === 'snowflake' && connectionTypeValue !== 'data_lake'
  ...
  return <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose} className="fixed inset-0 z-[100] flex justify-end bg-black/60 backdrop-blur-sm">
    <motion.aside initial={{x:440}} animate={{x:0}} exit={{x:440}} onClick={e => e.stopPropagation()} className="flex h-full w-full max-w-md flex-col border-l border-bg-border bg-bg-card shadow-2xl">
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-5">
        ...
        <Field label="Target"><select className="input-field" value={form.target} onChange={e => {const target=e.target.value; setForm(current => {const supported=normalizeChoice(target)==='snowflake' && normalizeChoice(current.connectionType)!=='data_lake'; return {...current,target,executionEngine:supported?current.executionEngine:'native',dbtDeploymentMode:supported?current.dbtDeploymentMode:'generate_only',forceDbtDeploy:supported?current.forceDbtDeploy:false}})}}><option>Databricks</option><option>Snowflake</option><option disabled>Fabric</option></select></Field>
        {snowflakeDatabaseTarget && <div className="space-y-3 rounded-lg border border-bg-border bg-bg-base p-3">
          <Field label="Snowflake Engine"><select className="input-field" value={executionEngineValue} onChange={e => {const executionEngine=e.target.value; setForm(current=>({...current,executionEngine,dbtDeploymentMode:executionEngine==='dbt'?current.dbtDeploymentMode:'generate_only',forceDbtDeploy:executionEngine==='dbt'?current.forceDbtDeploy:false}))}}><option value="native">Native</option><option value="dbt">dbt</option></select></Field>
          {executionEngineValue === 'dbt' && <Field label="dbt Mode"><select className="input-field" value={dbtDeploymentModeValue} onChange={e => {const dbtDeploymentMode=e.target.value; setForm(current=>({...current,dbtDeploymentMode,forceDbtDeploy:dbtDeploymentMode==='generate_and_deploy'?current.forceDbtDeploy:false}))}}><option value="generate_only">Generate Only</option><option value="generate_and_deploy">Generate & Deploy</option></select></Field>}
          {executionEngineValue === 'dbt' && dbtDeploymentModeValue === 'generate_and_deploy' && <label className="flex items-center gap-3 rounded-lg border border-bg-border bg-bg-card p-3 text-sm"><input type="checkbox" checked={!!form.forceDbtDeploy} onChange={e=>set('forceDbtDeploy',e.target.checked)}/>Force dbt deploy</label>}
        </div>}
        ...
        {connectionsLoading ? <p className="text-xs text-text-tertiary">Loading configurations...</p> : <>
          <Field label="Source Type"><select className="input-field" value={form.connectionType} onChange={e => {const connectionType=e.target.value; setForm(current=>{const supported=connectionType==='database' && normalizeChoice(current.target)==='snowflake'; return {...current, connectionType, connectionName:'', dbType:'', databaseName:'', integrationType:'', dataLakeName:'', executionEngine:supported?current.executionEngine:'native', dbtDeploymentMode:supported?current.dbtDeploymentMode:'generate_only', forceDbtDeploy:supported?current.forceDbtDeploy:false}})}}><option value="">Select source type...</option><option value="database">Database</option><option value="data_lake">Data Lake</option></select></Field>
