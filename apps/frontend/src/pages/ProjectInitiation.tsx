...
const EMPTY = {
  ...
  executionEngine: 'native', dbtDeploymentMode: 'generate_only', forceDbtDeploy: false,
}
...
export function isProjectFormValid(form) {
  const sourceConfigured = form.connectionType === 'database'
    ...
    : form.integrationType && (form.integrationType !== 'ADLS' || form.connectionName)
...
export function ProjectForm({ initial, connections, connectionsLoading, busy, onClose, onSave }) {
  ...
  const snowflakeSupportedTarget = String(form.target).toLowerCase() === 'snowflake' && (
    form.connectionType === 'database' ||
    (form.connectionType === 'data_lake' && String(form.integrationType).toLowerCase() === 'adls')
  )
  ...
  return <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose} className="fixed inset-0 z-[100] flex justify-end bg-black/60 backdrop-blur-sm">
    <motion.aside initial={{x:440}} animate={{x:0}} exit={{x:440}} onClick={e => e.stopPropagation()} className="flex h-full w-full max-w-md flex-col border-l border-bg-border bg-bg-card shadow-2xl">
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-5">
        ...
        <Field label="Target"><select className="input-field" value={form.target} onChange={e => {const target=e.target.value;setForm(current=>{const snowflakeSupported=String(target).toLowerCase()==='snowflake'&&(current.connectionType==='database'||(current.connectionType==='data_lake'&&String(current.integrationType).toLowerCase()==='adls'));return {...current,target,executionEngine:snowflakeSupported?current.executionEngine:'native',dbtDeploymentMode:snowflakeSupported&&current.executionEngine==='dbt'?'generate_and_deploy':'generate_only',forceDbtDeploy:false}})}}><option>Databricks</option><option>Snowflake</option><option disabled>Fabric</option></select></Field>
        ...
        {connectionsLoading ? <p className="text-xs text-text-tertiary">Loading configurations...</p> : <>
          <Field label="Source Type"><select className="input-field" value={form.connectionType} onChange={e => {const connectionType=e.target.value;setForm(current=>{const snowflakeSupported=String(current.target).toLowerCase()==='snowflake'&&connectionType==='database';return {...current,connectionType,connectionName:'',dbType:'',databaseName:'',integrationType:'',dataLakeName:'',executionEngine:snowflakeSupported?current.executionEngine:'native',dbtDeploymentMode:snowflakeSupported&&current.executionEngine==='dbt'?'generate_and_deploy':'generate_only',forceDbtDeploy:false}})}}><option value="">Select source type...</option><option value="database">Database</option><option value="data_lake">Data Lake</option></select></Field>
          {snowflakeSupportedTarget && <Field label="Snowflake Engine"><select className="input-field" value={form.executionEngine} onChange={e => {const executionEngine=e.target.value;setForm(current=>({...current,executionEngine,dbtDeploymentMode:executionEngine==='dbt'?'generate_and_deploy':'generate_only',forceDbtDeploy:false}))}}><option value="native">Native</option><option value="dbt">dbt</option></select><p className="mt-1 text-xs text-text-tertiary">{form.executionEngine === 'dbt' ? 'Deploy one combined Bronze, Silver, and Gold dbt project in Snowflake.' : 'Execute the generated Snowflake SQL directly without creating a dbt project.'}</p></Field>}
          {snowflakeSupportedTarget && form.executionEngine === 'dbt' && <Field label="dbt Run Mode"><input className="input-field cursor-not-allowed opacity-80" value="Deploy and build inside Snowflake" readOnly/><p className="mt-1 text-xs text-text-tertiary">Each fresh run adds a version containing Bronze, Silver, and Gold models, then runs dbt build.</p></Field>}
          ...
          {form.connectionType === 'data_lake' && <><Field label="Ingestion Type"><select className="input-field" value={form.integrationType} onChange={e=>{const c=lakeConnections.find(x=>x.integrationType===e.target.value); setForm({...form,integrationType:e.target.value,connectionName:String(c?.id||''),dataLakeType:c?.dataLakeSourceType||'',dataLakeName:c?.name||''})}}><option value="">Select ingestion...</option><option>ADLS</option><option>API</option></select></Field>{form.integrationType==='ADLS' && <Field label="Data Lake Name"><select className="input-field" value={form.connectionName} onChange={e=>{const c=lakeConnections.find(x=>String(x.id)===e.target.value);setForm({...form,connectionName:e.target.value,dataLakeName:c?.name||'',dataLakeType:c?.dataLakeSourceType||''})}}><option value="">Select data lake...</option>{lakeConnections.filter(c=>c.integrationType==='ADLS').map(c=><option key={c.id} value={String(c.id)}>{c.name}</option>)}</select></Field>}</>}
        </>}
      </div>
