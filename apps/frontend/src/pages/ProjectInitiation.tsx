  ...
  return <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={onClose} className="fixed inset-0 z-[100] flex justify-end bg-black/60 backdrop-blur-sm">
    <motion.aside initial={{x:440}} animate={{x:0}} exit={{x:440}} onClick={e => e.stopPropagation()} className="flex h-full w-full max-w-md flex-col border-l border-bg-border bg-bg-card shadow-2xl">
      ...
      <div className="flex-1 space-y-4 overflow-y-auto p-5">
        ...
        <Field label="Target"><select className="input-field" value={form.target} onChange={e => {const target=e.target.value;setForm(current=>{const snowflakeDatabase=String(target).toLowerCase()==='snowflake'&&current.connectionType==='database';return {...current,target,executionEngine:snowflakeDatabase?current.executionEngine:'native',dbtDeploymentMode:snowflakeDatabase&&current.executionEngine==='dbt'?'generate_and_deploy':'generate_only',forceDbtDeploy:false}})}}><option>Databricks</option><option disabled={form.connectionType === 'data_lake'}>Snowflake</option><option disabled>Fabric</option></select></Field>
        ...
        {connectionsLoading ? <p className="text-xs text-text-tertiary">Loading configurations...</p> : <>
          <Field label="Source Type"><select className="input-field" value={form.connectionType} onChange={e => {const connectionType=e.target.value;setForm(current=>{const snowflakeDatabase=connectionType==='database'&&String(current.target).toLowerCase()==='snowflake';return {...current,connectionType,target:connectionType==='data_lake'?'Databricks':current.target,connectionName:'',dbType:'',databaseName:'',integrationType:'',dataLakeName:'',executionEngine:snowflakeDatabase?current.executionEngine:'native',dbtDeploymentMode:snowflakeDatabase&&current.executionEngine==='dbt'?'generate_and_deploy':'generate_only',forceDbtDeploy:false}})}}><option value="">Select source type...</option><option value="database">Database</option><option value="data_lake">Data Lake</option></select></Field>
          {snowflakeDatabaseTarget && <Field label="Snowflake Engine"><select className="input-field" value={form.executionEngine} onChange={e => {const executionEngine=e.target.value;setForm(current=>({...current,executionEngine,dbtDeploymentMode:executionEngine==='dbt'?'generate_and_deploy':'generate_only',forceDbtDeploy:false}))}}><option value="native">Native</option><option value="dbt">dbt</option></select><p className="mt-1 text-xs text-text-tertiary">{form.executionEngine === 'dbt' ? 'Deploy one combined Bronze, Silver, and Gold dbt project in Snowflake.' : 'Execute the generated Snowflake SQL directly without creating a dbt project.'}</p></Field>}
          {snowflakeDatabaseTarget && form.executionEngine === 'dbt' && <Field label="dbt Run Mode"><input className="input-field cursor-not-allowed opacity-80" value="Deploy and build inside Snowflake" readOnly/><p className="mt-1 text-xs text-text-tertiary">Each fresh run adds a version containing Bronze, Silver, and Gold models, then runs dbt build.</p></Field>}
