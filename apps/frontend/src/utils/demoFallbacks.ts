...
function buildSilverScriptBody(tableName: string) {
  return [
    ...
    `QUARANTINE_TABLE = "silver.${tableName}_quarantine"`,
    'BUSINESS_KEYS = ["claim_id", "policy_id"]',
    ...
    'standardized_df = (',
    '    bronze_df',
    '    .withColumn("claim_id", F.trim(F.col("claim_id").cast("string")))',
    '    .withColumn("policy_id", F.trim(F.col("policy_id").cast("string")))',
    '    .withColumn("customer_id", F.trim(F.col("customer_id").cast("string")))',
    '    .withColumn("claim_status", F.upper(F.trim(F.col("claim_status").cast("string"))))',
    '    .withColumn("loss_date", F.to_date(F.col("loss_date")))',
...
function buildDemoScripts(layer: 'bronze' | 'silver' | 'gold', run: any) {
  ...
  const bodies = {
    ...
    silver: [
      buildSilverScriptBody(tableName),
    ].join('\n'),
    gold: [
